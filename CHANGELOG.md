...
### 2026-06-21 11:00 结构拆分规则写入 AGENTS
#### 摘要

- 在 `AGENTS.md` 新增代码和文档拆分规则。
- 明确 TS/TSX 文件超过 500 行必须优先拆分，400 行以上新增功能前需要评估拆分。
- 明确 `src/App.tsx`、`worker/index.ts` 和 `worker/shared.ts` 的职责边界，避免后续重新膨胀。
- 补充文档超过 800 行时的归档和拆分建议。

#### 说明

本次是项目维护规则更新，不改变运行时代码。目的是把当前批量拆分形成的模块边界固化到协作规则中，降低后续新增功能时重新堆回大文件的风险。
...
- 新增 `worker/routes/fans.ts`。
- 将粉丝列表、创建粉丝和编辑粉丝路由从 `worker/index.ts` 拆出。
- `worker/index.ts` 从约 1503 行降到约 1317 行，继续保留场次、票务、榜单和本场榜单路由。

#### 说明

本次是后端路由模块化的继续推进，不改变 API 路径和业务行为。粉丝资料相关逻辑独立后，后续可以继续拆直播场次、票务流水、本场榜单和定榜路由。
